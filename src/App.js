...
function App() {
  ...
  const [isChatbotOpen, setIsChatbotOpen] = useState(true); // State for chatbot visibility
  ...
  useEffect(() => {
    const handleMessage = (event) => {
      if (event.origin !== "https://dcabot.netlify.app") return; // Validate origin
      if (event.data.type === "TOGGLE_CHATBOT") {
        setIsChatbotOpen(event.data.isOpen);
      }
    };
  ...
  const handleEnd = () => {
    setIsChatbotOpen(false); // Close chatbot when chat ends
  };
        ...
        <StyledChatBotContainer>
          {isChatbotOpen && (
            <ChatBot
              ...
              hideUserAvatar={true}
              floatingIcon={
                <StyledChatBotFloatingIcon src={user1} alt="floaticon-logo" onClick={handleToggle} />
              }
              width="500px"
              handleEnd={handleEnd} // Handle end event to close chatbot
            />
          )}
          {!isChatbotOpen && (
            <StyledChatBotFloatingIcon src={user1} alt="floaticon-logo" onClick={handleToggle} />
